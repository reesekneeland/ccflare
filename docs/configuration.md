...
| `CCFLARE_LAST_RESORT_ACCOUNTS` | - | string | `CCFLARE_LAST_RESORT_ACCOUNTS=reese,backup` (comma-separated pay-per-use "extra usage" seat names; balanced normally until their own 5h quota fills, then used only as a last resort; default none) |
